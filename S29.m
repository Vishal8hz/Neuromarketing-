...
%% EEG Processing
productEEG = {};
myProduct = S29.(strcat('Page', int2str(PAGE_ID))).(strcat('Product', int2str(PRODUCT_ID)));
for iSegment = 1 : size(myProduct.EEG_segments, 1)
    ...
    productEEG{iSegment} = S29.EEG_clean.Data(:, startSample : endSample);
end
...
for iSegment = 1 : size(myProduct.ET_segments, 1)
    ...
    productET{iSegment} = S29.ET_clean.Data([3 6], startSample : endSample); % Pupil size
end
...
%% Alpha Filtering
EEGFs = S29.EEG_clean.Fs;
[b, a] = butter(3, [8 12]/(EEGFs/2));
filteredEEG = filtfilt(b, a, S29.EEG_clean.Data')';
...
for iPage = 1 : 6
    for iProduct = 1 : 24
        ...
        if S29.(strcat('Page',pageNo)).(strcat('Product',productNo)).ProductInfo.Bought == 1
...
productEEG = {};
myProduct = S29.(strcat('Page', int2str(indicesBought(1,1)))).(strcat('Product', int2str(indicesBought(1,2))));
for iSegment = 1 : size(myProduct.EEG_segments, 1)
    startSample = myProduct.EEG_segments(iSegment, 1);
    endSample = myProduct.EEG_segments(iSegment, 2);
